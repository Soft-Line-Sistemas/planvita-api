...
exports.BeneficiarioTipoController = void 0;
const beneficiariotipo_service_1 = require("../services/beneficiariotipo.service");
const logger_1 = __importDefault(require("../utils/logger"));
class BeneficiarioTipoController {
    constructor() {
        this.logger = new logger_1.default({ service: 'BeneficiarioTipoController' });
    }
    async getAll(req, res) {
        try {
            ...
            const service = new beneficiariotipo_service_1.BeneficiarioTipoService(req.tenantId);
            const result = await service.getAll();
            this.logger.info('getAll executed successfully', { tenant: req.tenantId });
            res.json(result);
        }
        catch (error) {
            this.logger.error('Failed to get all BeneficiarioTipo', error);
    ...
    async getById(req, res) {
        try {
            ...
            if (!result) {
                this.logger.warn(`BeneficiarioTipo not found for id: ${id}`, { tenant: req.tenantId });
            ...
            this.logger.error(`Failed to get BeneficiarioTipo by id`, error, { params: req.params });
            res.status(500).json({ message: 'Internal server error' });
        }
    }
    ...
    async update(req, res) {
        try {
            ...
            this.logger.error(`Failed to update BeneficiarioTipo`, error, {
                params: req.params,
                body: req.body,
            });
            res.status(500).json({ message: 'Internal server error' });
...
exports.BeneficiarioTipoController = BeneficiarioTipoController;
