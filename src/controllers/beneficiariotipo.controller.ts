...
export class BeneficiarioTipoController {
  private service = new BeneficiarioTipoService();
  ...
  async getById(req: Request, res: Response) {
    try {
      ...
      if (!result) {
        this.logger.warn(`BeneficiarioTipo not found for id: ${id}`);
  ...
  async update(req: Request, res: Response) {
    try {
      ...
      this.logger.error(`Failed to update BeneficiarioTipo`, error, { params: req.params, body: req.body });
      res.status(500).json({ message: 'Internal server error' });
    }
  }
