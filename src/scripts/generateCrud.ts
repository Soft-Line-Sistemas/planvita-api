...
const outputBase = path.join(process.cwd(), 'src');

async function generateCRUD() {
  const datamodel = fs.readFileSync(prismaSchemaPath, 'utf-8');
  ...
  for (const model of models) {
    ...
    const serviceDir = path.join(outputBase, 'services');
    const controllerDir = path.join(outputBase, 'controllers');
...
`;
    ...
    fs.writeFileSync(
      path.join(serviceDir, `${fileName}.service.ts`),
      serviceContent.trim() + '\n'
    );
...
fs.writeFileSync(
  path.join(controllerDir, `${fileName}.controller.ts`),
...
`;
    ...
    fs.writeFileSync(
      path.join(routesDir, `${fileName}.routes.ts`),
      routeContent.trim() + '\n'
    );
  }
